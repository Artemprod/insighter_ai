...
from costume_excepyions.path_exceptions import TelegramServerVolumePathExistingError, TelegramServerFileCantBeFoundError
import paramiko
...
class MediaFileManager(IMediaFileManager):
    ...
    @staticmethod
    async def get_shared_file_path(message: Message,
                                   file,
                                   bot: Bot) -> str:
        ...
        return os.path.join(MediaFileManager.base_path, bot.token, folder_name,
...
class TelegramMediaFileManager(MediaFileManager):
    ...
    async def get_media_file(self,
        ...
        try:
            ...
            self.log_file_info('error', message, None, "File hasn't been found in common docker volume")
...
class ServerFileManager(MediaFileManager):
    ...
    def __load_config(cls) -> Configs:
        env: Env = Env()
        env.read_env(".env")
        return cls.Configs(hostname=env('REMOTE_SERVER_HOST'),
    ...
    def __connect_server(self):
        ...
        try:
            client.connect(hostname=server_configs.hostname,
    ...
    def __execute_command_get_remote_file_size_mb(self,
                                                  file_path):
        client = self.__connect_server()
        try:
